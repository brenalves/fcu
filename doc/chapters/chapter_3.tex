...
\begin{enumerate}
    ...
    \item \textbf{Control Algorithm Implementation:} Development and tuning of PID or model-based control laws for pitch, roll, yaw, and altitude control. This ensures smooth and responsive flight performance. Tuning will be performed empirically, starting with the roll axis, by progressively increasing gains while monitoring for oscillation, first in simulation or a test bench, followed by real-world flight tests.
    %\\
    ...
    \textit{Expected result:} Verified flight stability and reliable autonomous or assisted control performance. This will be validated by analyzing the flight logs to plot the GNSS-defined planned path against the actual flight path and calculating the cross-track error
\end{enumerate}
